...
#approx    #filename = '../../Data/approx/S'+txt_S+'/phi'+DM+'/'+str(N)+'/'+'J2_J3=('+'{:5.4f}'.format(J2).replace('.','')+'_'+'{:5.4f}'.format(J3).replace('.','')+').csv'
    ...
    for i in range(N):
        ...
        if data[3] != 'True':
            ...
            exit()
        for d in data[6:]:                  #take only data values needed for the Hamiltonian -> from gap on, and without useless parameters
            if float(d) != 0.0:
                P.append(float(d))
    return P
...
def find_gap(data,N,arguments):
    ...
    kyg = np.linspace(0,1,N)
    kkg = np.ndarray((2,N,N),dtype=complex)
    for i in range(N):
        for j in range(N):
            kkg[0,i,j] = kxg[i]*2*np.pi
            kkg[1,i,j] = (kxg[i]+kyg[j])*2*np.pi/np.sqrt(3)
    P = []
    for p_ in range(len(data[2:])):
        if data[2+p_]:
            P.append(data[2+p_])
    m = 6
    funcs = {'SU2': matrix_SU2, 'TMD': matrix_TMD, 'SDM': matrix_SDM}
    matrix = funcs[arguments[-1]]
    Nk = matrix(P,N,arguments,data[1],kkg)
    min_energies = np.zeros((N,N))
    J_ = np.zeros((2*m,2*m))
    for i in range(m):
        J_[i,i] = -1
        J_[i+m,i+m] = 1
    #For each k in BZ grid diagonlize H and get min band
    for nx in range(N):
        for ny in range(N):
            Nk_ = Nk[:,:,nx,ny]
            try:
                Ch = LA.cholesky(Nk_)        #not always the case since for some parameters of Lambda the eigenmodes are negative
            except LA.LinAlgError:          #matrix not pos def for that specific kx,ky
                print('Error in Cholesky decomposition, neg minimum value at N=',N)
                return np.nan
            temp = np.dot(np.dot(Ch,J_),np.conjugate(Ch.T))    #we need the eigenvalues of M=KJK^+ (also Hermitian)
            min_energies[nx,ny] = LA.eigvalsh(temp)[m]      #only the smaller needed
    #####Gap by interpolation
    #Interpolate band and compute minimum by evaluating the fit on many more pts
    #func = RBS(kxg,kyg,min_energies)
    #KXG = np.linspace(0,1,1+20*12)
    #KYG = np.linspace(0,1,1+20*12)
    #en_values = func(KXG,KYG)
    #gap = np.amin(np.ravel(en_values))

    #####Gap by minimum value of the band
    gap = np.amin(np.ravel(min_energies))

    ####Possible plot
    #print('For N=',N,' foung gap=',gap,'\nmin energy was ',np.amin(min_energies.ravel()))
    #fig=plt.figure()
    #ax = fig.add_subplot(1,1,1,projection='3d')
    #X, Y = np.meshgrid(KXG,KYG)
    #X2,Y2 = np.meshgrid(kxg,kyg)
    #ax.plot_surface(X2,Y2,min_energies,cmap=cm.coolwarm)
    #ax.plot_surface(X,Y,en_values,cmap=cm.coolwarm)
    #plt.show()
    return gap

def matrix_TMD(P,N,arguments,L,kkg):
...
def matrix_SU2(P,N,arguments,L,kkg):
    #### vectors of 1nn, 2nn and 3nn
    a1 = (1,0)
    a2 = (-1,np.sqrt(3))
    a12p = (a1[0]+a2[0],a1[1]+a2[1])
...
def matrix_SDM(P,N,arguments,L,kkg):
    #### vectors of 1nn, 2nn and 3nn
    a1 = (1,0)
    a2 = (-1,np.sqrt(3))
    a12p = (a1[0]+a2[0],a1[1]+a2[1])
