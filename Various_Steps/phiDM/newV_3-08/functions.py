...
def eigG2_arr(k1, k2, params):
    ...
    #changing eta/eta_ gives different results
    if m == 3:
        G[0,1] = -2*J1*(eta*(-1)**(ans)-eta_*exp_k(k1,k2,0,1,m))
        G[0,2] = -2*J1*(eta*exp_k(k1,k2,0,1,m)-eta_*(-1)**(ans)*exp_k(k1,k2,-1,0,m))
        G[1,2] = -2*J1*(-eta_+eta*(-1)**(ans)*exp_k(k1,k2,-1,0,m))
