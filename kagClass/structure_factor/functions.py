...
def lattice(order,GT,angles):
    ...
    return L
#
def is_k_inside_BZ(K):
    if (K[1] < -np.sqrt(3)*K[0]-8*np.pi/np.sqrt(3) or
        K[1] >  np.sqrt(3)*K[0]+8*np.pi/np.sqrt(3) or
        K[1] > -np.sqrt(3)*K[0]+8*np.pi/np.sqrt(3) or
        K[1] <  np.sqrt(3)*K[0]-8*np.pi/np.sqrt(3) ):
        return False
    return True
#
def structure_factor(k,L,UC):
    resxy = 0
    resz = 0
    a1 = np.array([1,0])
    a2 = np.array([-1/2,np.sqrt(3)/2])       #Need to change this ad adjust lattices
    distx_UC = np.array([[0,1/2,1/4],[-1/2,0,-1/4],[-1/4,1/4,0]])
    disty_UC = np.array([[0,0,np.sqrt(3)/4],[0,0,np.sqrt(3)/4],[-np.sqrt(3)/4,-np.sqrt(3)/4,0]])
    for i1 in range(UC):
        for j1 in range(UC):
            for l1 in range(3):
                L1 = L[i1,j1,l1]
                ...
                for i2 in range(UC):
                    for j2 in range(UC):
                        for l2 in range(3):
                            ...
                            r2 = i2*a1+j2*a2
                            dist = np.zeros(2)
                            #dist[0] = i2 - i1 + j2/2 - j1/2 + distx_UC[l1,l2]#(l2%2)/2 - (l1%2)/2 + (l2//2)/4 - (l1//2)/4
                            #dist[1] = j2/2*np.sqrt(3) - j1/2*np.sqrt(3) + disty_UC[l1,l2]#(l2//2)/4*np.sqrt(3) - (l1//2)/4*np.sqrt(3)
