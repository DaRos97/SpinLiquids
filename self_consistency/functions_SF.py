...
def import_data(ans,filename):
    ...
    for i in range(N):
        ...
        if data[3] != 'True':
            ...
            exit()
        print('Gap value found: ',data[5])
        for d in data[6:]:                  #take only data values needed for the Hamiltonian -> from Lagrange multiplier on, and without useless parameters
            if float(d) != 0.0:
                P.append(float(d))
    return P
####
...
def Nk(K,par,args):
    ...
    ans_func = {'SU2': {'3x3': cf.ans_3x3, 'q0': cf.ans_q0, 'cb1': cf.ans_cb1, 'cb2': cf.ans_cb2, 'oct': cf.ans_oct},
            'TMD': {'3x3': cf.ans_3x3_TMD, 'q0': cf.ans_q0_TMD, 'cb1': cf.ans_cb1_TMD, 'cb2': cf.ans_cb2_TMD, 'oct': cf.ans_oct_TMD},
            }
