...
def get_data(Args,N):
    ...
    filename = '../Data/SC_data/S'+txt_S+'/phi'+DM+'/'+str(N)+'/'+'J2_J3=('+'{:5.4f}'.format(J2).replace('.','')+'_'+'{:5.4f}'.format(J3).replace('.','')+').csv'
    ...
    for i in range(N):
        ...
        if data[3] != '0':
            ...
            exit()
        for d in data[5:]:                  #take only data values needed for the Hamiltonian -> from gap on, and without useless parameters
            if float(d) != 0.0:
                P.append(float(d))
    return P
